...
import { PRIORITY_WEIGHTS, STORAGE_KEY_PERFORMANCE } from '../constants';
import { getMotivationalFeedback, getTaskSuggestions, generateVoiceReminderText } from '../services/geminiService';
import TaskModal from './TaskModal';
import FlowTelemetry from './FlowTelemetry';
import WellbeingParental from './WellbeingParental';
...
const CountdownTimer: React.FC<{ task: Task; onStatusChange: (status: TaskStatus) => void }> = ({ task, onStatusChange }) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      const now = new Date();
      if (!task.startTime || typeof task.startTime !== 'string' || !task.startTime.includes(':')) {
         setTimeLeft('0m 0s');
         return;
      }
...
const Dashboard: React.FC<DashboardProps> = ({ profile, tasks, onUpdateTasks }) => {
  ...
  useEffect(() => {
    socketObj = io('http://127.0.0.1:3002', { withCredentials: true });
    socketObj.on('taskSync', (updatedTask: Task) => {
  ...
  const updateStatus = (id: string, status: TaskStatus) => {
    ...
    const theTask = updated.find(t => t.id === id);
    if (theTask && socketObj) {
      socketObj.emit('taskUpdated', theTask);
    }

    if (status === 'Completed') {
      toast.success(`Protocol ${id} achieved. Score updated.`, {
        style: {
          borderRadius: '20px',
          background: '#10b981',
          color: '#fff',
  ...
  const handleVoiceReminder = async (task: Task) => {
    ...
    toast.promise(
      ...
      {
        loading: 'System articulating reminder...',
        success: 'Announcement complete.',
        error: 'Communication module failure.'
      },
      {
    ...
    <div className="space-y-12 pb-20">
      ...
      >
        <div className="space-y-2">
          <div className="flex items-center space-x-4">
            <h2 className="text-6xl font-black text-slate-800 dark:text-slate-100 tracking-tighter leading-none">
              {view === 'day' ? 'Perspective' : 'Insights'}
            </h2>
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl shadow-inner border border-transparent dark:border-slate-700">
              <button
                onClick={() => setView('day')}
                className={`px-6 py-2 rounded-xl text-xs font-black transition-all duration-300 transform active:scale-95 ${view === 'day' ? 'bg-white dark:bg-slate-700 shadow-md text-indigo-600 dark:text-indigo-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
              >
                Daily
              </button>
              <button
                onClick={() => setView('trends')}
                className={`px-6 py-2 rounded-xl text-xs font-black transition-all duration-300 transform active:scale-95 ${view === 'trends' ? 'bg-white dark:bg-slate-700 shadow-md text-indigo-600 dark:text-indigo-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
              >
                Yearly
              </button>
            </div>
      ...
      {view === 'day' ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          >
            <div className="glass rounded-[4rem] p-10 shadow-2xl border-white/60 dark:border-slate-700/60 dark:bg-slate-800/60 min-h-[600px] relative overflow-hidden">
              <div className="flex items-center justify-between mb-12">
                <h3 className="text-3xl font-black text-slate-800 dark:text-slate-100 tracking-tight">Timeline</h3>
              </div>
              ...
              {currentDayTasks.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-24 text-center animate-slide-up">
                  ...
                  <h4 className="text-2xl font-black text-slate-800 dark:text-slate-100">Void Detected</h4>
                  ...
                  <AnimatePresence>
                    {currentDayTasks.sort((a, b) => a.startTime.localeCompare(b.startTime)).map((task, i) => (
                      <motion.div
                        ...
                        animate={{ opacity: 1, x: 0, textDecoration: task.status === 'Completed' ? 'line-through' : 'none' }}
                        exit={{ opacity: 0, scale: 0.8, filter: 'blur(10px)' }}
                        style={{ filter: task.status !== 'Pending' ? 'grayscale(80%) opacity(70%)' : 'none' }}
                        transition={{ type: 'spring', stiffness: 300, damping: 24, delay: i * 0.05 }}
                        className="group relative glass bg-white/40 hover:bg-white/90 p-8 rounded-[3rem] border-white/80 transition-all hover-card-glow flex flex-col sm:flex-row items-center gap-8"
                      >
                        ...
                        <div className="flex-1 text-center sm:text-left">
                          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3">
                            <h4 className={`text-2xl font-black text-slate-800 dark:text-slate-100 leading-tight group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors duration-300 ${task.status === 'Completed' ? 'line-through text-slate-400 dark:text-slate-500' : ''}`}>{task.name}</h4>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {task.status === 'Pending' ? (
                            ...
                            <div className={`px-6 py-3 rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest shadow-sm transition-all duration-500 ${task.status.includes('Completed') ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100'
                              }`}>
                              {task.status}
                            </div>
                          )}
          ...
          >
            <div className="glass rounded-[3.5rem] p-10 shadow-2xl relative overflow-hidden group animate-float border-white/60 dark:border-slate-700/60 dark:bg-slate-800/80">
              ...
              <div className="flex items-end space-x-3 transform transition-transform duration-500 group-hover:scale-105">
                <span className="text-9xl font-black text-slate-800 dark:text-slate-100 tracking-tighter leading-none">{score}</span>
                <span className="text-4xl font-black text-indigo-300 mb-2">%</span>
              </div>
              
              {score === 0 && (
                <div className="mt-8 relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-500 to-purple-600 p-6 text-white shadow-lg border border-indigo-400/30 transform hover:scale-105 transition-all duration-300">
                  <div className="absolute -top-10 -right-10 w-32 h-32 bg-white/20 rounded-full blur-xl"></div>
                  <h4 className="font-black text-lg mb-2 flex items-center gap-2">
                    <span className="animate-bounce">⚡</span> Ignite Your Flow
                  </h4>
                  <p className="text-xs font-bold text-indigo-50 leading-relaxed max-w-[90%]">
                    Zero gravity detected. Even the biggest galaxies started from a single point. 
                    Initiate a protocol now to spark your momentum and get to work!
                  </p>
                </div>
              )}

              <div className="mt-8 p-8 bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-[2.5rem] border border-white dark:border-slate-700 italic font-bold text-slate-600 dark:text-slate-300 text-lg leading-relaxed shadow-inner">
                "{feedback}"
              </div>
            </div>

            <div className="glass rounded-[3rem] p-8 shadow-2xl border-white/60 dark:border-slate-700/60 dark:bg-slate-800/80 hover-card-glow transition-all duration-500">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-10 text-center">Outcome</h4>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      stroke="none"
                    >
                      {currentDayTasks.map((entry, index) => (
                        <Cell key={`cell-${index}`} />
                      ))}
                    </Pie>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {[
              ...
              <motion.div 
                ...
                className="glass p-10 rounded-[3rem] text-center border border-white/60 dark:border-slate-700/60 dark:bg-slate-800/80 shadow-xl hover:-translate-y-2 transition-all duration-500 hover:shadow-2xl hover-card-glow"
              >
                ...
                <div className="text-4xl font-black text-slate-800 dark:text-slate-100">{stat.value}</div>
              </motion.div>
            ))}
          </div>

          <div className="glass rounded-[4rem] p-12 shadow-2xl border-white/60 dark:border-slate-700/60 dark:bg-slate-800/60 transition-all duration-500 hover:shadow-indigo-100/50">
            <h4 className="text-3xl font-black text-slate-800 dark:text-slate-100 tracking-tight mb-12">Temporal Performance Arc</h4>
      ...
      <TaskModal
        ...
        onSave={(task) => {
          ...
          if (socketObj) socketObj.emit('taskUpdated', task); // sync new task too conceptually
